...
- **@live-design/mcp-server** (`packages/mcp-server/`) — MCP stdio server (using `@modelcontextprotocol/sdk`) + WebSocket bridge (port 24678). Spawns dev servers and manages review sessions. The overlay is injected by the vite-plugin, not the MCP server.
...
Build order matters: shared (no build) → overlay → vite-plugin → mcp-server. The overlay and vite-plugin can build in parallel. The mcp-server depends on shared. The vite-plugin depends on overlay (serves its dist files).
...
1. `start_session` — spawns dev server (or targets existing one), begins review session
...
5. `stop_session` — kills dev server, resets session
